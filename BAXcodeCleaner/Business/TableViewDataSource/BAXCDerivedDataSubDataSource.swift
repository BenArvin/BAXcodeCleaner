...
public class BAXCDerivedDataSubDataSource: BAXCTableViewSubDataSource {
    var derivedDataInfos: [(String, String?, String?, String?, Int, Bool)]? = nil
    var fullSize: Int = 0
    ...
    public override func setContent(for cell: NSTableCellView, row: Int, column: Int) {
        let (_, name, targetPath, lastUpdateTime, size, state) = self.derivedDataInfos![row]
        if column == 0 {
            ...
            if contentCell != nil {
                contentCell!.text = lastUpdateTime
            }
        } else if column == 2 {
            let contentCell: BAXCContentCell? = cell as? BAXCContentCell
            if contentCell != nil {
    ...
    public override func refresh() {
        let datas: ([(String, String?, String?, String?, Int)]?, Int) = BAXCDerivedDataTrashManager.datas() as! ([(String, String?, String?, String?, Int)]?, Int)
        ...
        if derivedDatas == nil {
            ...
            for (path, name, targetPath, lastUpdateTime, size) in derivedDatas! {
                if size == 0 {
                    continue
                }
                var finded: Bool = false
                if self.derivedDataInfos != nil {
                    for (oldPath, _, _, _, _, oldState) in self.derivedDataInfos! {
                        if oldPath == path {
                            newAppInfos.append((path, name, targetPath, lastUpdateTime, size, oldState))
                            finded = true
                            break
                        }
                    }
                ...
                if finded == false {
                    newAppInfos.append((path, name, targetPath, lastUpdateTime, size, false))
                }
            }
    ...
    public override func isAllChecked() -> Bool {
        ...
        for (_, _, _, _, _, state) in self.derivedDataInfos! {
            if state == false {
                allSelected = false
            }
        }
    ...
    public override func checkAll() {
        ...
        var index: Int = 0
        for (path, name, targetPath, lastUpdateTime, size, _) in self.derivedDataInfos! {
            self.derivedDataInfos![index] = (path, name, targetPath, lastUpdateTime, size, true)
            index = index + 1
        }
    }
    ...
    public override func uncheckAll() {
        ...
        var index: Int = 0
        for (path, name, targetPath, lastUpdateTime, size, _) in self.derivedDataInfos! {
            self.derivedDataInfos![index] = (path, name, targetPath, lastUpdateTime, size, false)
            index = index + 1
        }
    }
    ...
    public override func clean() {
        ...
        for (path, _, _, _, _, state) in self.derivedDataInfos! {
            if state == true {
    ...
    public override func contentForCopy(at row: Int) -> String? {
        ...
        let (path, name, _, _, size, _) = self.derivedDataInfos![row]
    ...
    public override func pathForOpen(at row: Int) -> String? {
        ...
        let (path, _, _, _, _, _) = self.derivedDataInfos![row]
        return path
    }
    ...
    public override func size() -> (Int, Int) {
        ...
        var total = 0
        var selected = 0
        for (_, _, _, _, size, state) in self.derivedDataInfos! {
            total = total + size
            if state == true {
